...
public class OrderRegistry {
	...
	public static void registerOrder(String modid, Object orderToPut) {
		...
		try
		{
            ...
            if(AuricMagickCraft.debugMode) System.out.println("The Order API has just registered " + 
            ((OrderBase)orderToPut).getUnlocName() + ".");
		}
		// Catch any error.
		catch ( Exception ex)
		{
			// Log in the FML error log (Mostly for debug purposes)
			FMLLog.log(Level.SEVERE, ex, "Caught an exception during an AMC" +
	...
	public static boolean removeOrder(String modid, OrderBase order)
	{
		...
		try
		{
			...
			ArrayList newList = orders.get(modid);
	...
	public static ArrayList<Object> getOrders()
	{
		...
		for (Enumeration e = getModIdsEnumerator(); e.hasMoreElements();)
		{
