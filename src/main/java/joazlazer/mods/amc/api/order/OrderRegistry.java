...
public class OrderRegistry {
	...
	public static void registerOrder(String modid, Object orderToPut) {
		...
		try
		{
            ...
            LogHelper.info("Registered order " +
                    ((OrderBase) orderToPut).getUnlocName());
		}
		// Catch any error.
		catch ( Exception ex)
		{
			// Log in the AMC error log (Mostly for debug purposes)
            LogHelper.error("Caught an exception during an AMC" +
	...
	public static OrderBase getOrder(String modid, int index)
	{
        ...
        int i = getOrderList().get(modid).size();
	...
	public static boolean removeOrder(String modid, OrderBase order)
	{
		...
		try
		{
			...
			// order to be deleted.
			assert !orders.get(modid).contains(order) : "AMC or one of its addons has tried to" +
					"delete an order that doesn't exist!";
		...
		try {
            ...
            for (Enumeration<String> e = getModIdsEnumerator(); e.hasMoreElements(); ) {
                ...
                for (int i = 0; i < orders.get(modidCurrent).size(); i++) {
                    if (orders.get(modidCurrent).get(i).getClass() ==  order) {
                        orders.get(modidCurrent).remove(i);
                        success = true;
                    }
                }
            }
	...
	/**
	 * A method for getting a list of ALL orders that are registered.
	 * @return
	 */
	 ...
	 * @param unlocName The unlocalized name of the order seared for.
	 * @return If it was found.
	 */
